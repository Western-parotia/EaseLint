...
object IncrementUtils {
    ...
    fun inject(project: Project, lintRequest: LintRequest) {
        when {
            project.gradle.startParameter.taskNames.find { it.contains(Constant.LINT_TASK_NAME_PREFIX) } != null -> {
    ...
    fun hasDiffFiles(project: Project): Boolean {
        ...
        checkFileList.clear()
        when {
            project.gradle.startParameter.taskNames.find {
                it.contains(Constant.LINT_TASK_TARGET)
            } != null -> {
                //增量扫描，通过git diff命令获取到新增和改动的文件
                checkIncrement(project)
            }
            project.gradle.startParameter.taskNames.find {
                it.contains(Constant.LINT_TASK_FULL)
            } != null -> {
